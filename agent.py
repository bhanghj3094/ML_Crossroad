...
# 최대 학습 횟수
MAX_EPISODE = 1000000   # 백만번
# 1000번의 학습마다 한 번씩 타겟 네트웍을 업데이트합니다.
TARGET_UPDATE_INTERVAL = 1000
# 4 프레임마다 한 번씩 학습합니다.
TRAIN_INTERVAL = 6    # 6번으로 해 장애물이 다가오는 모습을 전부 볼 수 있도록 합니다.
# 학습 데이터를 어느정도 쌓은 후, 일정 시간 이후에 학습을 시작하도록 합니다.
OBSERVE = 100

# action: 0: 좌, 1: 유지, 2: 우, 3: 위, 4: 아래    나머지 => 첫번째 차, 몫 => 두번째 차
NUM_ACTION = 25
SCREEN_WIDTH = 10
SCREEN_HEIGHT = 10
...
def train():
    ...
    # placeholder 는 선언 후, 그 다음 값을 전달하는 것.
    rewards = tf.placeholder(tf.float32, [None])
    # 앞은 name, reduce_mean()은 평균 // 인자에 ( , 0) : 열 | ( , 1) : 행 을 기준으로 한다.
...
def replay():
    ...
    #print("replay() width : %d, height : %d", SCREEN_WIDTH, SCREEN_HEIGHT)

    saver = tf.train.Saver()
    #print("replay() saver loaded")
    ckpt = tf.train.get_checkpoint_state('model')
    #print("replay() ckpt loaded")
    saver.restore(sess, ckpt.model_checkpoint_path)
    #print("replay() saver.restore loaded")

    # 게임을 시작합니다.
    for episode in range(MAX_EPISODE):
        #print("replay() episode: ", episode)
        terminal = False
        total_reward = 0
